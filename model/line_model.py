...
def init_emb2neg_index(negative, batch_size):
    ...
    """
    idx_list_u = list(range(batch_size)) * negative  # batch 内的相对idx
    idx_list_v = list(range(batch_size)) * negative
    random.shuffle(idx_list_v)  # batch 内随机找node作为负样本
...
def adam(grad, state_sum, nodes, lr, device):
    ...
    state_sum.index_add_(0, nodes, grad_sum)  # cpu  # 更新的时候映射回绝对idx
...
class SkipGramModel(nn.Module):
    ...
    def __init__(
        ...
        need_1st bool : need embedding for first-order proximity
        need_2nd bool : need embedding for second-order proximity
        ...
        if self.snd:
            ...
            init.constant_(self.snd_v_embeddings.weight.data, 0)  # 用0初始化
    ...
    def fast_neg_bp(self, emb_neg_u, emb_neg_v):
